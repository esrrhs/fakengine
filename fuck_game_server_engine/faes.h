...
static force_inline void faes(const stringc & strkey, const uint8_t * s_text, size_t s_len, uint8_t (&d_text)[AES_BUFF_LEN])
{
...
static force_inline stringc faes(const stringc & strkey, const stringc & s_text)
{
...
static force_inline void funaes(const stringc & strkey, const uint8_t * s_text, size_t s_len, uint8_t (&d_text)[AES_BUFF_LEN])
{
    uint8_t key[AES_KEY_BUFF_LEN];
    memset(key, 0, sizeof(key));
    memcpy(key, strkey.c_str(), Min<size_t>(strkey.size(), AES_KEY_BUFF_LEN));
...
static force_inline stringc funaes(const stringc & strkey, const stringc & s_text)
{
    uint8_t stext[AES_BUFF_LEN];
    memset(stext, 0, sizeof(stext));
    for (int32_t i = 0; i < AES_BUFF_LEN; i++)
