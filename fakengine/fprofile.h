...
inline void start_profiler(const char* fname)
{
}
...
inline profiledata g_ProfileData;  

inline void profiler_signal_handler(int sig, siginfo_t* sinfo, void* ucontext);
...
inline void profiledata::add(int depth, const void* const* stack)   
{  
    if (!enabled())   
    {  
        return;  
...
inline void profiledata::flushevicted()   
{  
    if (num_evicted_ > 0)   
    {  
...
inline bool profiledata::start(const char* fname)   
{  
    if (enabled())   
    {  
        return false;  
...
inline proc_maps_iterator::proc_maps_iterator(pid_t pid, buffer *buffer)   
{  
    init(pid, buffer, false);  
}  
      
inline void proc_maps_iterator::init(pid_t pid, buffer *b, bool use_maps_backing)   
{  
    pid_ = pid;  
...
inline proc_maps_iterator::~proc_maps_iterator()   
{  
    if (fd_ >= 0)   
    {  
        close(fd_);  
...
inline bool proc_maps_iterator::next(uint64_t *start, uint64_t *end, char **flags, uint64_t *offset, int64_t *inode, char **filename)   
{  
...
inline bool proc_maps_iterator::nextext(uint64_t *start, uint64_t *end, char **flags,  
                                uint64_t *offset, int64_t *inode, char **filename,  
                                uint64_t *file_mapping, uint64_t *file_pages,  
                                uint64_t *anon_mapping, uint64_t *anon_pages,  
                                dev_t *dev)   
...
inline int proc_maps_iterator::formatline(char* buffer, int bufsize,  
                                    uint64_t start, uint64_t end, const char *flags,  
